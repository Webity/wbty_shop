...
function ajaxsave(url, data, context, callback) {
	jQuery.ajax({
		url: url,
		data: data,
		dataType: 'json',
		...
		success: function(msg) {
			...
			if (callback) {
				console.log(msg.id);
				jQuery(document).trigger(callback, [msg.id]);
			}
		},
...
jQuery(document).ready(function($) {
	...
	$('#toolbar-save button').click(function(e) {
		e.preventDefault();
		...
		url = 'index.php?option=com_wbty_shop&task='+task;
	...
	$('.save-primary').click(function() {
		...
		$(this).after('<img src="'+window.juri_root+'/media/wbty_shop/img/load.gif" class="loading" />');
		task = $(this).closest('fieldset').attr('data-task');
		url = 'index.php?option=com_wbty_shop&task='+task;

		ajaxsave(url, data, $(this).parent().find('.loading'), 'save-primary');
		console.log(data);
	...
	$(document).on('click', '.inline-edit-trigger', function(e) {
		...
		url = window.juri_base + 'index.php?option=com_wbty_shop&task='+ $this.attr('data-controller') + '.edit';
	...
	$('body').on('click', '.inline-edit-save-btn', function(e) {
		...
		url = window.juri_base + 'index.php?option=com_wbty_shop&task='+ $fields.attr('data-controller') + '.ajax_save';
		
		if ($fields.attr('data-append')) {
			url = url + '&' + $fields.attr('data-append');
		}
		
		$(this).closest('.edit-form').after($('<p style="text-align:center;" data-update="'+$(this).closest('.edit-form').attr('data-update')+'" data-fieldset="'+$(this).closest('.edit-form').attr('data-fieldset')+'"><img src="'+window.juri_root+'media/wbty_shop/img/load.gif" /></p>'));
