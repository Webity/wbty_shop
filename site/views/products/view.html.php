	...
	protected function addToolbar()
	{
		...
		if (file_exists($formPath)) {
            if ($canDo->get('core.create')) {
				$bar->appendButton( 'Standard', 'new', 'New', 'product.add', false );
		    }
		    if ($canDo->get('core.edit') && isset($this->items[0])) {
				$bar->appendButton( 'Standard', 'edit', 'Edit', 'product.edit', false );
				
				if (isset($this->items[0]->checked_out)) {
					$bar->appendButton( 'Standard', 'checkin', 'Check In', 'products.checkin', false );
				}
		    }
			
			if ($canDo->get('core.edit.state') && isset($this->items[0]->state)) {
				if ($this->state->get('filter.state') != -2) { 
					$bar->appendButton( 'Standard', 'trash', 'Trash', 'products.trash', false );
				} else {
					$bar->appendButton( 'Standard', 'publish', 'Publish', 'products.publish', false );
				}
			}
		...
		if ($canDo->get('core.admin')) {
			//JToolBarHelper::preferences('com_wbty_shop');
		}
